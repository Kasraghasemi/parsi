...
parsi.compositional_synthesis(sub_sys,horizon,initial_order=2,step_size=0.1 ,order_max=100,algorithm='fast')
...
parsi.compositional_synthesis(sub_sys,horizon,initial_order=5,step_size=0.1 ,order_max=100,algorithm='fast')

# Plotting the results

# fig2, axs2 = plt.subplots(number_of_subsystems)
...
#     pp.visualize([sub_sys[i].omega,*sub_sys[i].viable], ax = axs2[i],fig=fig2, title='',equal_axis=True)

#     axs2[i].plot( path[:,0], path[:,1] ,color='b')

#     # drawing the parameterized sets
#     for step in range(1,horizon):
#         pp.visualize( [ pp.zonotope( G= np.dot( sub_sys[i].omega.G , np.diag( sub_sys[i].alpha_x[step-1]) ) ,  x= sub_sys[i].x_nominal[step] , color = 'yellow') ] , ax = axs2[i], fig = fig2 ,title='' , equal_axis=True)
